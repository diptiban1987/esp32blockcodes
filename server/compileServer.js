...
function compileSketch(inoCode) {
  ...
  try {
    ...
    // Prefer split (4 individual files) — matches exactly what Arduino IDE does and is
    // always generated by arduino-cli. Merged binary is only used as a last resort
    // because flashing it at 0x0000 combined with pako string handling caused
    // flash corruption (invalid header: 0xbfc3bfc3) when individual files are available.
    const binaryType = (bootBin && partBin && appBin) ? "split" : (mergedBin ? "merged" : "app");
